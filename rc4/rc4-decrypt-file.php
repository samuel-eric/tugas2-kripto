...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit'])) {
  if ($_FILES['cipher']['error'] === UPLOAD_ERR_OK) {
    ...
    $cipherText = $ext === 'txt' ? base64_decode(file_get_contents($_FILES['cipher']['tmp_name'])) : file_get_contents($_FILES['cipher']['tmp_name']);
    $key = $_POST['key'] ?? '';
...
<div class="card m-5" style="width: 25rem;">
  <div class="card-body">
    ...
    <h5 class="card-title fs-3 text-center">RC4 Decryption with File</h5>
    <hr>
    ...
    <?php if (isset($filename) && isset($output)) : ?>
      ...
      <?php endif ?>
      <p>
        <a href="/uploads/<?= $filename ?>" download>Download decrypted file</a>
      </p>
    <?php else : ?>
      <div class="alert alert-info text-center">
        <strong>Please upload encrypted file and key</strong>
      </div>
    <?php endif ?>
  </div>
</div>
